      ...
      <div className="animate-fade-in-up" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-muted-foreground">Explore virtual labs and experiment guides.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {experiments.map((exp, index) => (
            <Card key={index} className="overflow-hidden flex flex-col h-full hover:shadow-lg hover:border-primary transition-all group relative">
               {isFaculty && (
              ...
              <a href={exp.youtubeUrl} target="_blank" rel="noopener noreferrer" className="block">
                <div className="relative w-full h-48 bg-secondary border-b flex items-center justify-center text-muted-foreground">
                   {exp.imageUrl ? (
                     <Image src={exp.imageUrl} alt={exp.title} layout="fill" objectFit="cover" />
                   ) : (
